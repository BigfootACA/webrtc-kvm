...
void V4L2Device::LoadControlsConfig(YAML::Node&obj){
	if(!obj.IsDefined())return;
	if(!obj.IsMap())throw InvalidArgument("bad controls config");
	for(const auto&kv:obj)
		controls[kv.first.as<uint32_t>()]=kv.second.as<int32_t>();
}
...
void V4L2Device::LoadGenericConfig(YAML::Node&obj){
	...
	if(auto v=obj["controls"])LoadControlsConfig(v);
